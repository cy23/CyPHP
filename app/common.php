 ...
 *
 * @package      	LTPHP
 * @author          Cuiyi QQ:120428037 <120428037@qq.com>
 * @copyright     	Copyright (c) 2011-2016
 * @license         /license.txt
 * @version        	LowThinkPHP企业网站管理系统 v2.3 2016-06-28 $
 */
if (!defined("Ltphp")) exit("Access Denied");
...
function sendmail($tomail, $subject, $body, $config = '') {
    ...
    //$mail->From ="cy23@vip.qq.com";
    //$mail->FromName = "LowThinkPHP企业建站系统";
    $mail->SetFrom($config['mail_from'], $config['site_name']); //发送者邮箱
    $mail->AddAddress($tomail); //可同时发多个
    //$mail->AddReplyTo('cy23@vip.qq.com', 'LowThinkPHP'); //回复到这个邮箱
    ...
    if ($name == 'Lang') {
        ...
        foreach ($list as $key => $r) {
            if ($r['groupid'] == 6) {
                ...
                if (APP_LANG) $memberconfig_temp[$r['lang']][$r['varname']] = $r['value'];
                else $memberconfig[$r['varname']] = $r['value'];
            } else {
                if (APP_LANG) if ($r['lang']) {
                    $temp[$r['lang']][$r['varname']] = $r['value'];
                } else {
                    $data[$r['varname']] = $r['value'];
                } else $data[$r['varname']] = $r['value'];
            }
        }
        if (APP_LANG) {
            $lang = F('Lang');
        ...
        foreach ((array)$langs as $r) $lang1[] = $r['mark'];
...
function getlang($have = '') {
    if ($have) {
        ...
        else $lang = $_REQUEST['l'] ? $_REQUEST['l'] : C('URL_LANG');
    } else {
        if (strcasecmp(GROUP_NAME, 'Admin') == 0) $lang = C('URL_LANG') != LANG_NAME ? LANG_NAME : '';
        else $lang = $_REQUEST['l'] && C('URL_LANG') != $_REQUEST['l'] ? $_REQUEST['l'] : '';
    }
    return $lang;
}
...
function thumb($f, $tw = 300, $th = 300, $autocat = 0, $nopic = 'nopic.jpg', $t = '') {
    ...
    if (function_exists('imagecopyresampled')) imagecopyresampled($timg, $fimg, 0, 0, 0, 0, $tw, $th, $fw, $fh);
    else imagecopyresized($timg, $fimg, 0, 0, 0, 0, $tw, $th, $fw, $fh);
    if ($tmp == 'gif' || $tmp == 'png') {
...
function sql_split($sql, $tablepre) {
    if ($tablepre != "lt_") $sql = str_replace("lt_", $tablepre, $sql);
